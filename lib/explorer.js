...
function rpcCommand(params, cb, count, last_error) {

  let retry_limit=100;

  if (typeof (count) === 'undefined') {
    // console.log("Starting rpcCommand")
    count = 0
  }else{
    count = count + 1
  }
  if (count>retry_limit) {
    console.log('There was an RpcError. Check your console. ' + count, last_error);
    console.log("Retry limit reached.")
    exit()
  }

  // console.log("Trying client.command " + count)
  client.command([{method: params[0].method, parameters: params[0].parameters}], function (err, response) {
    if (err) {
      // console.log('There was an rpcCommand. Check your console. ' + count, err);
      rpcCommand(params, cb, count, err)
    } else {
      if (response[0].name == 'RpcError') {
        // console.log('There was an RpcError. Check your console. ' + count, response);
        rpcCommand(params, cb, count, response)
      }
      // console.log("Finished rpcCommand")
